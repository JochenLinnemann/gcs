...
/** The detailed editor for {@link SplittermondSpell}s. */
public class SplittermondSpellEditor extends BaseSpellEditor<SplittermondSpell> {
    private JTextField mBaseSkillNameField;
    private JTextField mPrerequisiteSpellsCountField;

    /**
     * Creates a new {@link SplittermondSpell} {@link RowEditor}.
     *
     * @param spell The row being edited.
     */
    protected SplittermondSpellEditor(SplittermondSpell spell) {
        super(spell);

        Container content      = new JPanel(new ColumnLayout(2));
        Container fields       = new JPanel(new ColumnLayout());
        Container wrapper1     = new JPanel(new ColumnLayout(2));
    ...
    protected Container createPointsFields() {
        ...
        mDifficultyCombo.setEnabled(false);
    ...
    protected void recalculatePoints(JTextField pointsField) {
        int prereqCount = Integer.parseInt(mPrerequisiteSpellsCountField.getText());
        int points      = SplittermondSpell.calculatePoints(prereqCount);
        pointsField.setText(Integer.toString(points));
    }

    protected void recalculateLevel(JTextField levelField) {
        SkillLevel level = SplittermondSpell.calculateLevel(mRow.getCharacter(), mNameField.getText(), mBaseSkillNameField.getText());
    ...
    protected void adjustForSource(Object src) {
        if (src == mPrerequisiteSpellsCountField) {
            if (mPointsField != null) {
                recalculatePoints(mPointsField);
            }
        } else if (src == mBaseSkillNameField) {
            if (mLevelField != null) {
